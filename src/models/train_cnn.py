"""
CNN Model Training and Evaluation Module

This module provides functions for preprocessing, training, evaluating, and saving a CNN model for sentiment analysis.
...
Functions:
    ...
    - train_cnn_model_and_tokenizer(train_df, val_df, model, learning_rate=0.001, epochs=10, batch_size=32, max_length=128, vocab_size=10000, stop_words=False):
      Trains a CNN model and returns the trained model, history, and tokenizer.
...
def _preprocess_data(df, max_length=128, vocab_size=10000, stop_words=False):
    """
    Preprocesses text data for training a CNN model.
...
def train_cnn_model_and_tokenizer(train_df, val_df, model, learning_rate=0.001, epochs=10, batch_size=32, max_length=128, vocab_size=10000, stop_words=False):
    """
    Trains an CNN model on a given dataset and returns the trained model, training history, and tokenizer.

    This function performs the following steps:
    - Preprocesses the training and validation datasets.
    - Tokenizes text data and converts it into numerical sequences.
    - Compiles and trains the provided CNN model.
    ...
    Args:
        ...
        model (tf.keras.Model): A Keras CNN model to be trained.
...
def evaluate_model(model, tokenizer, test_df, max_length=128, stop_words=False):
    """
    Evaluates a trained CNN model on a test dataset and prints a classification report.

    This function tokenizes and pads the text data from the test dataset using a pre-trained tokenizer,
    makes predictions using the trained CNN model, and generates a classification report showing the
    model's performance.
